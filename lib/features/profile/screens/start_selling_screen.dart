...
class _StartSellingScreenState extends State<StartSellingScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text("تسجيل الدخول مطلوب", style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontWeight: FontWeight.w900, fontSize: 20, letterSpacing: 0.5)),
              const SizedBox(height: 12),
              Text("عفواً، لا يمكنك $featureName كزائر. قم بتسجيل الدخول لتستمتع بجميع مميزات GEAR UP! 🚗✨", textAlign: TextAlign.center, style: TextStyle(color: isDark ? Colors.white70 : AppColors.textSecondary, fontSize: 14, height: 1.6)),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("رسوم نشر الإعلان", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: isDark ? Colors.white : Colors.black87)),
                const SizedBox(height: 8),
                Text("لضمان جدية الإعلانات، سيتم خصم مبلغ 50 جنيهاً مصرياً. يرجى اختيار طريقة الدفع في الخطوة التالية.", textAlign: TextAlign.center, style: TextStyle(color: isDark ? Colors.white70 : AppColors.textSecondary, fontSize: 14, height: 1.5)),
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          if (marketCubit.userPaymentMethods.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('الرجاء إضافة طريقة دفع أولاً لإتمام العملية'), backgroundColor: Colors.orange));
                        ...
                        child: const Text("موافق للمتابعة", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
            ...
            return SafeArea(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text("اختر طريقة الدفع", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: isDark ? Colors.white : Colors.black87)),
                    const SizedBox(height: 16),
                    if (methods.isEmpty) ...[
                      const Center(child: Text("لا توجد طرق دفع مسجلة.", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold))),
                      const SizedBox(height: 16),
                    ] else ...[
                      ConstrainedBox(
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        ...
                        label: const Text("إضافة طريقة دفع جديدة", style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold)),
  ...
  Future<void> _executeActualPayment(Map<String, dynamic> selectedMethod) async {
    final marketCubit = context.read<MarketCubit>();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('جاري الاتصال ببوابة الدفع، يرجى الانتظار...'), duration: Duration(seconds: 3)));

    Map<String, dynamic> paymentResult;

    if (selectedMethod['type'] == 'Card') {
      paymentResult = await PaymobManager().getCardPaymentUrl(50);
    } else {
      String walletNum = selectedMethod['subtitle'].toString().split('\n').last.trim();
      paymentResult = await PaymobManager().getWalletPaymentUrl(50, walletNum);
    }
    ...
    if (paymentResult['success'] == true) {
      ...
      String itemName = _companyController.text.trim().isNotEmpty ? "${_companyController.text.trim()} ${_modelController.text.trim()}" : "إعلان جديد";

      if (paymentSuccess == true) {
        await marketCubit.addTransactionRecord(title: "$itemName - رسوم نشر", amount: "-50 EGP", status: "Completed", isPositive: false);
        _executePublishing();
      } else {
        await marketCubit.addTransactionRecord(title: "$itemName - رسوم نشر", amount: "-50 EGP", status: "Failed", isPositive: false);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إلغاء أو فشل الدفع! وتم تسجيل العملية في السجل.'), backgroundColor: Colors.red));
      }
    } else {
  ...
  void _confirmDelete(BuildContext context, VoidCallback onConfirm) {
    showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text('تأكيد الحذف', style: TextStyle(fontWeight: FontWeight.bold)), content: const Text('هل أنت متأكد من أنك تريد مسح طريقة الدفع هذه؟'), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('إلغاء')), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red), onPressed: () { Navigator.pop(ctx); onConfirm(); }, child: const Text('حذف', style: TextStyle(color: Colors.white)))]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    String screenTitle = isEditing ? (_isAiCar ? 'تعديل بيانات السيارة' : 'تعديل الإعلان') : (AppLang.tr(context, 'start_selling_title') ?? 'Start Selling');
          ...
          if (!isEditing)
            IconButton(
              ...
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم مسح المسودة والبدء من جديد')));
              },
            )
        ],
      ),
      body: BlocConsumer<MarketCubit, MarketState>(
        listener: (context, state) {
          if (state is AddCarSuccess) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isEditing ? 'تم التعديل بنجاح' : (AppLang.tr(context, 'publish_success') ?? 'تم النشر بنجاح')), backgroundColor: Colors.green));
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(isEditing ? 'عدل البيانات اللي محتاجها واضغط حفظ' : (AppLang.tr(context, 'choose_item_type') ?? 'Choose item type'), style: TextStyle(color: isDark ? Colors.white70 : AppColors.textSecondary, fontSize: 14)),

                if (isEditing && _isCoreFieldsLocked) Container(margin: const EdgeInsets.only(top: 16), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orangeAccent.withOpacity(0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.orangeAccent.withOpacity(0.5))), child: const Row(children: [Icon(Icons.info_outline_rounded, color: Colors.orangeAccent, size: 20), SizedBox(width: 10), Expanded(child: Text('مر 24 ساعة على نشر الإعلان. لا يمكن تغيير نوع أو ماركة أو موديل الإعلان لحماية المنصة.', style: TextStyle(color: Colors.orangeAccent, fontSize: 12, fontWeight: FontWeight.bold)))])), const SizedBox(height: 24),
                ...
                GestureDetector(
                  ...
                  child: Container(padding: const EdgeInsets.symmetric(vertical: 18), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 6))]), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [state is AddCarLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Text(isEditing ? 'حفظ التعديلات' : (AppLang.tr(context, 'publish_listing') ?? 'Publish'), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 0.5))])),
